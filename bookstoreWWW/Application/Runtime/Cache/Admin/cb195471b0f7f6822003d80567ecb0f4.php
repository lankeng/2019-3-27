...
<body>
<div class="title" style="color:red;">商品修改页面</div>
<div class="data-edit clear">
	<form method="post" enctype="multipart/form-data">
	<table>
	    <tr><th>商品编号：</th><td><input style="height:25px;" type="text" name="identifier" value="<?php echo ($goods["identifier"]); ?>"/></td></tr>
		<tr><th>商品名：</th><td><input style="height:25px;" type="text" name="gname" value="<?php echo ($goods["gname"]); ?>"/></td></tr>
		<tr><th>商品价格：</th><td><input style="height:25px;" type="text" name="price" value="<?php echo ($goods["price"]); ?>"/></td></tr>
		<tr><th>商品库存：</th><td><input style="height:25px;" type="text" name="stock" value="<?php echo ($goods["stock"]); ?>"/></td></tr>
		<tr><th>商品图片：</th><td><input type="file" name="thumb" class="file" /></td></tr>
		<tr><th>是否上架：</th><td><select name="status">
			<option value="yes" <?php if(($goods["status"]) == "yes"): ?>selected<?php endif; ?> >是</option>
			<option value="no" <?php if(($goods["status"]) == "no"): ?>selected<?php endif; ?>>否</option>
		</select></td></tr>
		<tr><th>首页推荐：</th><td><select name="is_hot">
			<option value="yes" <?php if(($goods["is_hot"]) == "yes"): ?>selected<?php endif; ?> >是</option>
			<option value="no" <?php if(($goods["is_hot"]) == "no"): ?>selected<?php endif; ?>>否</option>
		</select></td></tr>
		<tr><th>商品描述：</th><td><script type="text/plain" id="myEditor" name="description"><?php echo ($goods["description"]); ?></script></td></tr>
		<?php if(is_array($attr)): foreach($attr as $key=>$v): ?><tr><th><?php echo ($v["aname"]); ?>：</th><td><?php if(isset($v["a_def_val"]["1"])): ?><select name="attr[<?php echo ($v["aid"]); ?>]">
					...
					<?php if(is_array($v["a_def_val"])): foreach($v["a_def_val"] as $key=>$vv): ?><option value="<?php echo ($vv); ?>" <?php if(($v["avalue"]) == $vv): ?>selected<?php endif; ?>><?php echo ($vv); ?></option><?php endforeach; endif; ?>
				</select>
			<?php else: ?>
				<input type="text" name="attr[<?php echo ($v["aid"]); ?>]" value="<?php echo ($v["avalue"]); ?>"/><?php endif; ?></td></tr><?php endforeach; endif; ?>
		<tr class="tr_btn center">
			<td colspan="2"><input  style="height:30px;background-color:red;color:white;text-algin:center;line-height:20px;" type="submit" value="确定" /><input  style="height:30px;background-color:red;color:white;text-algin:center;line-height:20px;" type="reset" value="清除" /></td>
		</tr>
	</table>
	</form>
</div>

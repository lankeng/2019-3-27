		...
		<div id="rightContent">
			...
			<div class="tab-content">
				...
				<div role="tabpanel" class="tab-pane active" id="order">
					...
					<div class="data-div">
						<div class="row tableHeader bookmg">
							<div class="col-xs-1">
								书籍名称
							</div>
							<div class="col-xs-1">
						...
						<div class="tablebody">
							<form action="<?php echo U('Admin/Order/order');?>" method="GET">
								<?php if(is_array($select)): foreach($select as $key=>$v): ?><div class="row bookmg">
										<div class="col-xs-1">
											<?php echo ($v["bookname"]); ?>
										</div>
										<div class="col-xs-1">
										...
										<div class="col-xs-2">
											<?php echo ($v["area"]); echo ($v["address"]); ?>
										</div>
										<div class="col-xs-1">
											<?php echo ($v["deliverytime"]); ?>
										</div>
										<div class="col-xs-1">
											<?php echo ($v["lmessage"]); ?>
										</div>
										...
										<div class="col-xs-1">
											<button type="button" class="btn btn-success btn-xs upstate" data-toggle="modal" data-target="#changeChar" data-orderid="<?php echo ($v["orderid"]); ?>" data-state="<?php echo ($v["state"]); ?>">修改</button>
											<button type="button" class="btn btn-danger btn-xs delorder" data-toggle="modal" data-target="#deleteorder" data-orderid="<?php echo ($v["orderid"]); ?>">删除</button>
										</div>
					...
					<div class="modal fade" id="changeChar" role="dialog" aria-labelledby="gridSystemModalLabel">
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								...
								<form  action="<?php echo U('Admin/Order/uporder');?>" method="POST" enctype="multipart/form-data">
									<div class="modal-body">
										<div class="container-fluid">
											<div class="form-horizontal">
												<div class="form-group">
													<label for="sLink" class="col-xs-3 control-label bookfz ">配送状态：</label>
													<div class="col-xs-8 ">
														<select style="margin-top: 7px" class="selstate" name="state">
															<option value="待发货">待发货</option>
															<option value="包裹等待收揽">包裹等待收揽</option>
															<option value="已发货">已发货</option>
														</select>
														<input type="text" class="uporderid" style="display: none" name="uporderid">
													</div>
												</div>
											</div>
										</div>
					...
					<div class="modal fade" id="deleteorder" role="dialog" aria-labelledby="gridSystemModalLabel">
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								...
								<form  action="<?php echo U('Admin/Order/delorder');?>" method="POST">
									<div class="modal-body">
										<div class="container-fluid">
											确定要删除该订单？删除后不可恢复！
											<input type="text" name="delorderid" class="delorderid" style="display: none;">
										</div>
									</div>
									<div class="modal-footer">
										<button type="button" class="btn btn-xs btn-white" data-dismiss="modal">取 消</button>
	...
	<script>
			...
			$(document).ready(function() {
				...
				$(".delorder").click(function(){
					var delid = $(this).data("orderid");
					$(".delorderid").val(delid);
				})
